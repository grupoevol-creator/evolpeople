// ===== EVOL PEOPLE — Frontend (app.js v8) =====
// v8: suporte a multiplas unidades do socio, busca/cadastro de salario,
...
const TITULOS = {
  ...
  testePratico: 'Teste Prático', treinamentos: 'Treinamentos',
  universidadeEvol: 'Universidade Evol', feedback: 'Feedback',
  avaliacaoExperiencia: 'Avaliação de Experiência', escala: 'Escala',
  cargosSalarios: 'Cargos & Salários', episFardamento: 'EPIs & Fardamento',
  dossie: 'Dossiê', pdi: 'Meu PDI'
};

async function api(acao, dados) {
  try {
    const r = await fetch(WEBAPP_URL, { method: 'POST', body: JSON.stringify({ acao, dados: dados || {} }) });
    return await r.json();
  } catch (e) { console.error(e); return { sucesso: false, erro: 'Falha de conexão' }; }
}
...
function montarMenu() {
  ...
  if ((USUARIO.perfil === 'SOCIO' || USUARIO.perfil === 'LIDER') && USUARIO.unidades && USUARIO.unidades.length > 1) {
    ...
    html += `<div class="campo" style="padding:8px 12px;">
      <label style="font-size:12px;color:#666;">Unidade</label>
      <select id="seletorUnidade" onchange="trocarUnidade(this.value)">${ops}</select>
    </div><hr style="border:none;border-top:1px solid #eee;margin:4px 0;"/>`;
  }
...
function navegarPara(pagina) {
  ...
  if (fn) fn(); else renderApp('<div class="card"><p>Tela nao implementada.</p></div>');
}
...
async function telaDashboard() {
  ...
  const vagasArr = d && d.vagasAbertasPorCargo ? Object.entries(d.vagasAbertasPorCargo).map(([cargo, qtd]) => ({ cargo, qtd })) : [];
  renderApp(`
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:16px;margin:24px 32px;">
      <div class="card"><h3>Headcount</h3><p style="font-size:28px;font-weight:700;">${d && d.headcountPorUnidade ? Object.values(d.headcountPorUnidade).reduce((a,b)=>a+b,0) : 0}</p></div>
      ...
      <div class="card"><h3>Projeção Orçamento</h3><p style="font-size:20px;font-weight:700;">${formatarMoeda(d && d.projecaoOrcamento ? d.projecaoOrcamento : 0)}</p></div>
    ...
    <div class="card"><h3>Vagas abertas por cargo</h3>
      ${vagasArr.length ? `<table class="tabela"><thead><tr><th>Cargo</th><th>Vagas</th><th>Subtotal</th></tr></thead><tbody>
      ...
      </tbody></table>` : '<p>Nenhuma vaga aberta.</p>'}
    </div>
    <div class="card"><h3>Experiência vencendo (15 dias)</h3>
      ${d && d.periodosExperienciaVencendo && d.periodosExperienciaVencendo.length ? `<ul>${d.periodosExperienciaVencendo.map(p => `<li>${esc(p.nome)} - ${p.periodo} - vence ${esc(p.dataVencimento)}</li>`).join('')}</ul>` : '<p>Nenhum periodo vencendo.</p>'}
    </div>`);
}
...
async function telaVagas() {
  ...
  renderApp(`<div class="card">
    ...
    ${vagas.length ? `<table class="tabela"><thead><tr><th>Vaga</th><th>Unidade</th><th>Setor</th><th>Status</th><th>Candidato</th></tr></thead><tbody>
      ${vagas.map(v => `<tr><td>${esc(v.vaga)}</td><td>${esc(v.unidade)}</td><td>${esc(v.setor)}</td><td><span style="background:${(v.status||'').toUpperCase()==='ABERTA'||(v.status||'').toUpperCase()==='SELECAO'?'#facc15':'#22c55e'};padding:2px 8px;border-radius:6px;font-size:12px;">${esc(v.status)}</span></td><td>${esc(v.candidato||'-')}</td></tr>`).join('')}
    </tbody></table>` : '<p>Nenhuma vaga.</p>'}
  </div>`);
}

function mostrarFormVaga() {
  const c = document.getElementById('formVagaContainer'); if (!c) return;
  const uPre = UNIDADE_SELECIONADA || '';
  c.innerHTML = `<div class="card" style="margin-top:16px;">
    <h3>Nova Vaga</h3><form class="formulario" onsubmit="return salvarVaga(event)">
    <div class="grid-form">
      ...
      <div class="campo"><label>Unidade</label><input type="text" id="vagaUnidade" value="${esc(uPre)}" required/></div>
    ...
    <button type="button" class="btn btn-secundario" onclick="document.getElementById('formVagaContainer').innerHTML='';">Cancelar</button>
    </form></div>`;
}
...
async function telaColaboradores() {
  ...
  renderApp(`<div class="card"><h2>Colaboradores</h2>
    ${cols.length ? `<table class="tabela"><thead><tr><th>ID</th><th>Nome</th><th>Funcao</th><th>Unidade</th><th>Admissao</th></tr></thead><tbody>
      ${cols.map(l => `<tr><td>${esc(l[0])}</td><td>${esc(l[1])}</td><td>${esc(l[2])}</td><td>${esc(l[6])}</td><td>${esc(l[18]||'')}</td></tr>`).join('')}
    </tbody></table>` : '<p>Sem colaboradores.</p>'}
  </div>`);
}
...
function telaTestePratico() {
  ...
  renderApp(`<div class="card">
    ...
    <form class="formulario" onsubmit="return salvarTestePratico(event)">
      ...
      <button type="submit" class="btn btn-destaque">Salvar Teste Prático</button>
    </form>
  </div>`);
}
...
async function buscarSalarioTP() {
  ...
  if (res.sucesso && res.salario) {
    ...
    document.getElementById('tpFixo').value = '';
    document.getElementById('tpCompl').value = '';
    document.getElementById('tpTotal').value = '';
...
async function salvarTestePratico(e) {
  ...
  if (res.sucesso) {
    if (dados.recomendacao === 'APROVADO') alert('Teste salvo! Se a vaga foi encontrada no backend, foi encerrada automaticamente.');
    else alert('Teste salvo!');
    telaTestePratico();
  } else alert(res.erro || 'Erro');
  return false;
}
...
async function telaEscala() {
  ...
  renderApp(`<div class="card">
    ...
    <form class="formulario" onsubmit="return gerarEscalas(event)">
      ...
      <button type="submit" class="btn btn-destaque">Gerar Escala</button>
    </form>
    ...
    <form class="formulario" onsubmit="return salvarFeedback(event)">
      <div class="grid-form">
        <div class="campo"><label>Modelo</label>
          <select id="fbModelo" onchange="trocarModeloFB()">
            <option value="90dias">Feedback 90 dias</option>
            <option value="trimestral">Desempenho Trimestral</option>
            <option value="lideranca">Feedback de Lideranca</option>
            <option value="pontual">Feedback Pontual</option>
          </select>
        </div>
        <div class="campo"><label>Colaborador</label><select id="fbColab">${ops}</select></div>
      </div>
      <div id="fbCampos"></div>
      <button type="submit" class="btn btn-destaque">Salvar Feedback</button>
    </form>
  </div>`);
  trocarModeloFB();
}
...
async function salvarFeedback(e) {
  ...
  const pts = [];
  ['fb1','fb2','fb3'].forEach(id => { const el = document.getElementById(id); if (el) pts.push(el.value); });
  const dados = {
...
function telaTreinamentos() {
  ...
  renderApp(`<div class="card">
    ...
    <form class="formulario" onsubmit="return salvarTreinamento(event)">
      ...
      <button type="submit" class="btn btn-destaque">Salvar Treinamento</button>
    </form>
  </div>`);
}
...
function telaUniversidadeEvol() {
  ...
  renderApp(`<div class="card">
    <h2>Universidade Evol - ${esc(ctx)}</h2>
    <p>Trilhas de desenvolvimento disponiveis.</p>
    <div class="grid-form">
...
async function telaAvaliacaoExperiencia() {
  ...
  renderApp(`<div class="card">
    <h2>Avaliacao de Experiencia</h2>
    <form class="formulario" onsubmit="return salvarAvaliacaoExp(event)">
      ...
      <div class="campo"><label>Observacoes</label><textarea id="aeObs"></textarea></div>
      <button type="submit" class="btn btn-destaque">Salvar Avaliacao</button>
    </form>
  </div>`);
}
...
function telaCargosSalarios() {
  renderApp(`<div class="card">
    <h2>Cargos & Salarios</h2>
    <p>Consulta de tabela de cargos.</p>
...
// ===== EPIS & FARDAMENTO =====
function telaEpisFardamento() {
  renderApp(`<div class="card"><h2>EPIs & Fardamento</h2>
    <p>Consulte o estoque na tela de Estoque (menu RH) ou registre entregas.</p>
  </div>`);
}

// ===== DOSSIÊ =====
function telaDossie() {
  renderApp(`<div class="card"><h2>Dossie</h2>
    <div class="grid-form">
...
function telaPdi() {
  renderApp(`<div class="card"><h2>Meu PDI</h2>
    <form class="formulario" onsubmit="return salvarPdi(event)">
      <div class="campo"><label>Objetivo</label><input type="text" id="pdiObj" required/></div>
      <div class="campo"><label>Acoes</label><textarea id="pdiAcoes"></textarea></div>
      <div class="campo"><label>Prazo</label><input type="date" id="pdiPrazo"/></div>
      <button type="submit" class="btn btn-destaque">Salvar PDI</button>
    </form>
  </div>`);
}

async function salvarPdi(e) {
  e.preventDefault();
  alert('PDI: funcionalidade em desenvolvimento no backend.');
  return false;
}
